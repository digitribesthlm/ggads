...
version: alpha
name: portal-design
description: Portal with a confident brand voice — anchored by a deep navy hero band ({colors.brand-navy}), a signature purple primary CTA ({colors.primary}), and a rich palette of pastel-tinted feature cards. The system uses an Inter-based typeface across every UI surface.
...
Portal presents itself as a campaign management platform through a confident, illustration-rich brand voice. The homepage opens with **"Welcome to the platform."** rendered centered over a deep navy hero band ({colors.brand-navy}), decorated with decorative elements. The signature **purple pill primary CTA** ({colors.primary})  sits at the visual center, paired with an outlined "Request a demo" secondary. Below the buttons, a workspace UI mockup card (the ) breaks out of the hero band with a deep diffuse drop shadow.

Below the hero, the page cycles through a distinctive sequence of feature sections: a dense sticky-note "Keep work moving 24/7" panel with red/blue/green/purple/teal status icons; a **bold yellow** ({colors.card-tint-yellow-bold}) "Ask your on-demand assistants" banner card flanked by orange/rose/mint pastel feature tiles showing assistant UI mockups; and a "Bring all your work together" 3-column grid with brand-colored mockups (sky-blue tutorial card, light  calendar, brown/rust testimonial slate). The pricing page renders 4 tiers (Free / Plus / Business / Enterprise) horizontally with one tier featured (purple-bordered) and a dense feature comparison table running below.

The system uses a Inter typeface (Inter-based) across every UI surface — humanist-geometric character that pairs naturally with the colorful illustrations. Buttons are `{rounded.md}` (8px) rectangles, NOT pills —. Cards use `{rounded.lg}` (12px) consistently.

**Key Characteristics:**
- Deep navy hero band ({colors.brand-navy}) with scattered sticky-note dots + mesh wire decorative illustrations
- **Signature purple** ({colors.primary}) primary CTA  button color
- Real workspace UI mockup card embedded in the hero with deep drop shadow
...
- Inter (Inter-based) across every UI surface
...
> Source pages: .com/ (homepage), /enterprise, /product/ai, /product/agents, /startups, /pricing. Token coverage was identical across all six pages.

### Brand & Primary
- ** Purple** ({colors.primary}): Signature primary CTA color — the unmistakable  pill button. Reserved for the dominant CTA only.
...
- **Charcoal** ({colors.charcoal}): Body emphasis ('s signature warm-charcoal)
...
### Font Family
**Inter** (primary):Inter-based variable typeface. Fallbacks: Inter, -apple-system, system-ui, 'Segoe UI', Helvetica, sans-serif. Humanist-geometric character used across every UI surface.

### Hierarchy

| Token | Size | Weight | Line Height | Letter Spacing | Use |
|---|---|---|---|---|---|
| `{typography.hero-display}` | 80px | 600 | 1.05 | -2px | Hero ("Welcome to the platform") |
...
's geometry is sober-editorial — `{rounded.md}` (8px) buttons distinguish it from pill-button-everywhere brands.
...
**`workspace-mockup-card`** — Embedded  workspace UI mockup on hero band ().
- Background `{colors.canvas}`, rounded `{rounded.lg}`, border `1px solid {colors.hairline}`, deep shadow `rgba(15, 15, 15, 0.20) 0px 24px 48px -8px`. Carries actual product UI mock.
...
- Left: +"Products / Resources / Pricing" links.
- Right:  purple button + "Log in" link.
...
- 6-column link grid (Product / Resources / Company / Legal).
...
- Apply `{rounded.md}` (8px) to buttons consistently —  uses rectangles, not pills
- Apply `{rounded.lg}` (12px) to all card families
- Maintain Inter across every UI surface
...
- Don't use pill-shaped buttons; 's geometry is rectangular-sober
...
- Don't replace Inter with a generic Inter
